...
the first time it shows up. Wherever we did something specific to this
project, we've replaced the real values (server address, passwords, website
address) with placeholders like `YOUR_DOMAIN.com` — swap those for your own.
...
- **Server / VPS**: a computer that isn't yours, sitting in a data center
  ...
  for Virtual Private Server — think of it as renting one slice of a much
  bigger computer. This is the thing your website actually lives on.
...
This lives in a file at `.github/workflows/ci.yml`, and GitHub runs it
automatically — you don't have to remember to do it yourself. Think of it
as a spell-checker for code that also checks whether the plumbing actually
connects.
...
1. Buy a VPS from any provider (Hostinger, DigitalOcean, Linode, AWS, etc.)
   running **Ubuntu** (a free, very common operating system for servers).
   You'll get an **IP address** (a string of numbers like `123.45.67.89`
   that uniquely identifies your server on the internet) and a password or
   a way to generate a secure key to log in.
...
1. On the server, download a copy of the project's code:
...
2. The project needs some settings that shouldn't be stored in the code
   itself — API keys, database passwords, and similar secrets. These live
   in **environment files** (`.env`-style files) that you create directly
   on the server and never commit to GitHub. Two files, both created by
   hand with a text editor (`nano filename` opens a simple in-terminal text
   editor — type your content, then `Ctrl+O`, Enter, `Ctrl+X` to save and
   exit):
   ...
   **A real mistake we hit**: one of these values was a randomly-generated
   ...
   plain letters and numbers (e.g. a hexadecimal generator), not one that
   can output symbols.
...
1. **Buy a domain** (e.g. from Namecheap, GoDaddy, Google Domains) if you
   don't have one — this is a completely separate purchase from the VPS.
2. **Point the domain at your server**: in your domain provider's control
   ...
   "when someone looks up `YOUR_DOMAIN.com`, send them to
   `YOUR_VPS_IP`." This can take a few minutes to a few hours to take
   effect worldwide (called "DNS propagation").
3. **Run a reverse proxy**. This is a piece of software that sits in front
   ...
   this same server also hosts an unrelated automation tool), reuse it
   rather than running a second one; two programs can't both listen on the
   same port.
4. Configure the reverse proxy to route your domain to your app's
   ...
   *wrong* one when deciding where to send traffic — so it could complete
   ...
   telling the reverse proxy which network to use. If you ever see "HTTPS
   connects fine, but the page never loads, with no errors in any log,"
   check for this exact situation.
...
1. **Create a dedicated login key just for this purpose** — not your
   ...
   Generate it directly on the server (this matters — copying a
   multi-line security key through a chat window or an email can silently
   corrupt it; typing the generation command directly where it'll be used
...
2. **Give GitHub the private half, safely**. GitHub Actions has a
   ...
   **A real problem we hit repeatedly**: pasting a multi-line security key
   ...
   lines) into GitHub's secret box kept getting silently corrupted — a
   line break lost here, an invisible character added there — and every
...
3. **Write the automation steps** (a file at
   `.github/workflows/deploy.yml`, similar to the CI checks file from
   Part 3, but this one actually logs into the server and runs the update
   commands from Part 5, step 3, automatically).
...
| Server / VPS | A rented, always-on computer your website lives on |
...
| Docker / container | A self-contained, portable package of an app + everything it needs |
